...
async function recordAndGetReadings() {
  ...
  for (const asset of assets) {
    ...
    const score  = parseFloat(Math.max(0, Math.min(100, base + rand(-3, 3))).toFixed(1));
    const rul    = parseFloat((score * 0.9 + rand(0, 10)).toFixed(1));
    ...
    await query(
      ...
      { asset_id: asset.id, vibration, temperature, pressure,
        health_score: score, rul, mtbf, status }
    );

    readings.push({
      id: asset.id, name: asset.name, location: asset.location,
      healthScore: score, rul, mtbf, status,
...
async function getLatestReadings() {
  const rows = await query(`
    ...
    INNER JOIN (
      ...
      GROUP BY asset_id
    ) latest ON latest.asset_id = r.asset_id
            AND latest.latest   = r.recorded_at
  `);
